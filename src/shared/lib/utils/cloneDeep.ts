import { Indexed, SymbolsIndexed } from './types.ts';
  ...
  return (function _cloneDeep(
    ...
    if (item instanceof Object) {
      ...
      Object.getOwnPropertySymbols(item).forEach(
        (s) =>
          ((copy as SymbolsIndexed)[s] = _cloneDeep(
            (item as SymbolsIndexed<T>)[s],
          )),
      );
      ...
      Object.keys(item).forEach(
        (k) => ((copy as Indexed)[k] = _cloneDeep((item as Indexed<T>)[k])),
      );
